...
router.get('/pokemonsNumber/:number',async(req,res)=>{
    try {
      const number = req.params.number
      const pokemon= await Pokemon.find({number})
      // console.log("eerroorr", pokemon)
        res.send({pokemon,msg:"pokemon successfully "})
    } catch (error) {
        console.log(error)
        res.status(400).send({msg:"error while getting one pokemon"})
    }
    })
//get pokemon by name
router.get('/pokemons/:name', async(req,res)=>{
  try {
    ...
    console.log("erroorr", name)
    const pokemon= await Pokemon.find({name :  { $regex: `${name}`, $options: 'i'}})
    console.log("pokemon eerroorr", pokemon)
      res.send({pokemon,msg:"pokemon successfully "})
  } catch (error) {
      console.log(error)
      res.status(400).send({msg:"error while getting one pokemon"})
